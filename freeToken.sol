...
contract Free is ERC20Detailed, Ownable {
    ...
    string public _name = "Freedom Token";
    ...
    constructor(address _swapRouter,address _freeDaoAddress,address _autoLiquidityReceiver,address _firePit,uint256 _initSupply) ERC20Detailed(_name,_symbol, uint8(DECIMALS)) Ownable() {
...
//        }
    ...
    function takeFee(
        ...
        if (sender != pair) {
            ...
            IFreeDao(freeDaoAddress).setDaoReward(gonAmount.mul(daoFee).div(feeDenominator).div(_gonsPerFragment));
        }
        ...
        if (sender == pair){
            ...
            for (uint8 i=0;i<_parents.length;i++){
                uint256 _parentFee = gonAmount.mul(50).div(10000);
                if (i==0){
                    _parentFee = gonAmount.mul(400).div(10000);
                }
                if(i==1){
                    _parentFee = gonAmount.mul(200).div(10000);
                }
